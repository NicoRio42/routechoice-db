import type { LoggatorEvent } from '../models/loggator-api/logator-event';
...
export function buildRunnersTracksFromLoggatorData(
	...
	const pointsMap: Record<string, { lat: number; lon: number; time: number }[]> = {};
	...
	runners.forEach((runner) => {
		...
		const runnerInfos = loggatorEvent.competitors.find(
			(competitor) => `loggator-${competitor.device_id}` === runner.trackingDeviceId
		);
