<script lang="ts">
	...
	$: {
		...
		firstRunner = runners.find((r) => r.id === selectedRunners[0]) ?? runnersWithTracks[0];

		secondRunner = runners.find((r) => r.id === selectedRunners[1]) ?? runnersWithTracks[1];
		...
		if (firstRunner !== undefined && secondRunner !== undefined) {
			...
			if (firstRunnerCoords.length < secondRunnerCoords.length) {
				lines = firstRunnerCoords.map((point, i) => [point, secondRunnerCoords[i]]);
			} else {
				lines = secondRunnerCoords.map((point, i) => [point, firstRunnerCoords[i]]);
			}
		}
	}
</script>
...
{#if firstRunner !== undefined && secondRunner !== undefined}
	...
	{#each lines as line, index}
		<LineString color={index % 2 === 0 ? 'red' : 'blue'} coords={line} width={2} />
	{/each}
{/if}
