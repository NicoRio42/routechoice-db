<script lang="ts" context="module">
	...
	export function getNewRunnerOffset(rnrId: string): Promise<[number, boolean]> {
		runnerId.set(rnrId);
		showDialog.set(true);
...
{#if $showDialog}
	<form on:submit|preventDefault={handleSubmit}>
		<div class="wrapper">
			<button type="button" on:click={() => offset--} class="counter-button">-</button>
		...
		<div class="wrapper">
			<label class="all-runners" for="all-runners"
				...
				<input type="checkbox" name="all-runners" bind:checked={applyToAllRunners} />
			</label>

			<button type="button" class="outline cancel" on:click={handleCancel}>Cancel</button>
