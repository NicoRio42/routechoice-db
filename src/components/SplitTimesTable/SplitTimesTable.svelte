              ...
              <div class="tooltip-container">
                ...
                <span class="tooltip tooltip-top">
                  ...
                  {#if runner.totalTimeLost !== 0}
                    <div>
                      {`Time loss: ${secondsToPrettyTime(
                        runner.totalTimeLost
                      )}`}
                    </div>
                  {/if}
                </span>
          ...
          {#each runner.legs as leg}
            <td class={leg.isMistake ? "mistake" : ""}>
              ...
              {#if leg.timeOverall !== null}
                <div
                  class="tooltip-container {rankToCSSClass(leg.rankOverall)}"
                >
                  {`${secondsToPrettyTime(leg.timeOverall)} (${
                    leg.rankOverall
                  })`}

                  <span class="tooltip"
                    >{`+ ${secondsToPrettyTime(leg.timeBehindOverall)}`}</span
                  >
                </div>
              {/if}
            </td>
